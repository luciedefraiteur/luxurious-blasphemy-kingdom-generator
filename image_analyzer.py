...
class ImageAnalyzer:
    def __init__(self, model_name: str = "Salesforce/blip2-opt-2.7b", force_cpu: bool = False):
        # Pour RTX 2070, BLIP-2 2.7B est trop gros, utilisons CPU pour le modèle mais GPU pour les calculs
        self.device = "cpu" if force_cpu else ("cuda" if torch.cuda.is_available() else "cpu")
        self.compute_device = "cuda" if torch.cuda.is_available() and not force_cpu else "cpu"

        print(f"🔮 Initialisation ImageAnalyzer")
        print(f"   Modèle sur: {self.device}")
        print(f"   Calculs sur: {self.compute_device}")

        # Nettoyer la mémoire GPU
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
            memory_total = torch.cuda.get_device_properties(0).total_memory / 1024**3
            print(f"🔧 Mémoire GPU totale: {memory_total:.1f} GB")

        # Charger le modèle BLIP-2 en CPU pour éviter OOM
        print("📦 Chargement du modèle BLIP-2 en mode hybride...")
        self.processor = Blip2Processor.from_pretrained(model_name)

        # Charger le modèle en CPU pour éviter les problèmes de mémoire
        self.model = Blip2ForConditionalGeneration.from_pretrained(
            model_name,
            torch_dtype=torch.float32,  # Float32 en CPU
            low_cpu_mem_usage=True
        )

        # Garder le modèle en CPU
        self.model = self.model.to("cpu")

        print("✅ Modèle BLIP-2 chargé avec succès (mode hybride CPU/GPU)")
    ...
    def analyze_image(self, image_path: Path, custom_prompt: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for prompt in prompts:
                # Traiter l'image avec le prompt (toujours en CPU pour éviter OOM)
                inputs = self.processor(image, prompt, return_tensors="pt")

                # Générer la description (modèle en CPU, plus stable)
                with torch.no_grad():
                    generated_ids = self.model.generate(
                        **inputs,
                        max_length=100,  # Restauré car on a plus de mémoire en CPU
                        num_beams=5,     # Restauré pour meilleure qualité
                        do_sample=False,
                        early_stopping=True
                    )
    ...
    def analyze_sigil(self, sigil_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            for prompt in sigil_prompts:
                inputs = self.processor(image, prompt, return_tensors="pt")  # Pas de .to(device)
